...
Gera 4 gráficos com insights sobre sintomas, comportamento e transmissão
"""
...
class AnalisadorPNADCovid:
    ...
    def grafico_1_sintomas_evolucao(self):
        ...
        colunas_sintomas = [
            col for col in self.df.columns if any(
        ...
        if not colunas_sintomas:
            logger.warning(
                "Nenhuma coluna de sintomas encontrada, criando dados simulados...")
                ...
                for col in colunas_sintomas:
                    try:
                        pct = (
                            ...
                            len(df_mes) *
                            100)
                        nome_sintoma = col.replace('_', ' ').title()[
                            :20]  # Limitar tamanho
                        resultado.append({
                            'Mês': MESES_NOMES[mes],
        ...
        if not resultado.empty:
            ...
            pivot.plot(
                ...
                width=0.8)
            ax.set_title(
                'Prevalência de Sintomas Clínicos por Mês\nPNAD-COVID-19 (Brasil)',
                fontsize=14,
                fontweight='bold',
                pad=20)
            ...
            plt.savefig(
                ...
                bbox_inches='tight')
            logger.info(
                "[OK] Gráfico 1 salvo: relatorios/graficos/01_sintomas_evolucao.png")
            plt.close()
    ...
    def grafico_2_internacao_sintomas(self):
        ...
        ax.set_title(
            'Taxa de Internação Hospitalar por Sintoma Principal\nPNAD-COVID-19',
            fontsize=14,
            fontweight='bold',
            pad=20)
        ...
        plt.savefig(
            ...
            bbox_inches='tight')
        logger.info(
            "[OK] Gráfico 2 salvo: relatorios/graficos/02_taxa_internacao_sintomas.png")
        plt.close()
    ...
    def grafico_3_comportamento_evolucao(self):
        ...
        ax.set_title(
            'Evolução Temporal: Adesão às Medidas de Prevenção\nPNAD-COVID-19 (Brasil)',
            fontsize=14,
            fontweight='bold',
            pad=20)
        ...
        plt.savefig(
            ...
            bbox_inches='tight')
        logger.info(
            "[OK] Gráfico 3 salvo: relatorios/graficos/03_comportamento_evolucao.png")
        plt.close()
    ...
    def grafico_4_indice_transmissao_beta(self):
        ...
        # Subplot 1: Beta
        # Vermelho forte, vermelho fraco, verde
        colors1 = ['#d32f2f', '#ff6b6b', '#4caf50']
        ...
        ax1.set_ylabel(
            ...
            fontweight='bold')
        ax1.set_title(
            'Parâmetro β: Dinâmica de Transmissão',
            fontsize=12,
            fontweight='bold')
        ax1.set_ylim(0, 1)
        ...
        bars2 = ax2.bar(resultado['Mês'], resultado['R₀ (Reprodução Básica)'],
                        color=colors1, edgecolor='black', linewidth=1.5)
        ax2.set_ylabel(
            'R₀ (Número de Reprodução Básica)',
            fontsize=11,
            fontweight='bold')
        ax2.set_title(
            'Número de Reprodução Básica (R₀)',
            fontsize=12,
            fontweight='bold')
        ax2.set_ylim(0, 7)
        ...
        plt.savefig(
            ...
            bbox_inches='tight')
        logger.info(
            "[OK] Gráfico 4 salvo: relatorios/graficos/04_indice_transmissao_beta.png")
        plt.close()
