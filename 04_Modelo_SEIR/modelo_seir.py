...
class ModeloSEIR:
    ...
    def calcular_metricas(self, S, E, I, R):
        ...
        taxa_ataque = (R[-1] / self.N) * 100  # % da população que foi infectada
...
class CenariosSEIR:
    ...
    def cenario_1_baseline_maio(self):
        ...
        logger.info(f"[OK] Taxa de ataque final: {metricas['taxa_ataque_final']:.1f}%")
    ...
    def cenario_3_adaptacao_novembro(self):
        ...
        beta = self.dados.get('beta_novembro', 0.35)  # Menor transmissão (mais medidas)
    ...
    def cenario_4_novo_surto_mitigacao(self):
        ...
        logger.info(f"[OK] Redução vs Cenário 1: {((1 - metricas['pico_infectados']/self.resultados['maio']['metricas']['pico_infectados'])*100):.1f}%")
        logger.info(f"[OK] Taxa de ataque: {metricas['taxa_ataque_final']:.1f}%")
    ...
    def plotar_comparacao_cenarios(self):
        ...
        fig.suptitle('Modelo SEIR: Comparação de Cenários', fontsize=16, fontweight='bold')
        ...
        for cenario_key, ax, titulo in cenarios_plot:
            if cenario_key in self.resultados:
                ...
                ax.plot(t, S/1_000_000, label='Suscetíveis (S)', linewidth=2, color='blue')
                ax.plot(t, E/1_000_000, label='Expostos (E)', linewidth=2, color='orange')
                ax.plot(t, I/1_000_000, label='Infectados (I)', linewidth=2, color='red')
                ax.plot(t, R/1_000_000, label='Recuperados (R)', linewidth=2, color='green')
                
                pico = np.max(I)
                dia_pico = np.argmax(I)
                ax.plot(dia_pico, pico/1_000_000, 'r*', markersize=15, label=f'Pico: {pico/1_000_000:.1f}M')
        ...
        plt.savefig('relatorios/graficos/05_seir_cenarios_completos.png', dpi=300, bbox_inches='tight')
        logger.info("[OK] Gráfico salvo: relatorios/graficos/05_seir_cenarios_completos.png")
        plt.close()
    ...
    def plotar_comparacao_infectados(self):
        ...
        cores = {'maio': '#FF6B6B', 'agosto': '#FFA500', 'novembro': '#4CAF50', 'novo_surto_mitigado': '#2196F3'}
        ...
        for cenario_key, cor in cores.items():
            if cenario_key in self.resultados:
                ...
                ax.plot(t, I/1_000_000, label=label, linewidth=2.5, color=cor)
        ...
        ax.grid(True, alpha=0.3)
        ax.axhline(y=1, color='red', linestyle='--', alpha=0.5, label='1 milhão infectados')
        
        plt.tight_layout()
        plt.savefig('relatorios/graficos/06_seir_comparacao_infectados.png', dpi=300, bbox_inches='tight')
        logger.info("[OK] Gráfico salvo: relatorios/graficos/06_seir_comparacao_infectados.png")
        plt.close()
