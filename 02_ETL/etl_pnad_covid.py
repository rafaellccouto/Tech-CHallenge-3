...
Prepara os dados XLSX de Dados_Base/ para análise no modelo SEIR e análises hospitalares
"""
...
def carregar_dados_xlsx(file_path: str, mes: int) -> pd.DataFrame:
    ...
    try:
        ...
        colunas_disponiveis = [col for col in df.columns if col not in ['Unnamed: 0', 'index']]
        ...
        if len(colunas_disponiveis) > 0:
            ...
            logger.info(f"[OK] Colunas após seleção: {len(df_filtrado.columns)}")
            return df_filtrado
        else:
...
def limpar_dados(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"[OK] Limpeza concluída. Linhas: {len(df_limpo):,}, Colunas: {len(df_limpo.columns)}")
...
def main():
    ...
    for mes in MESES_ANALISE:
        ...
        if file_path and os.path.exists(file_path):
            ...
            logger.error(f"[ERRO] Arquivo não encontrado para {MESES_NOMES[mes]}")
