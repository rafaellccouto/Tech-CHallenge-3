...
def main():
    ...
    if not run_script('./02_ETL/etl_pnad_covid.py', 'ETL (Limpeza de dados XLSX)'):
        print_error("Falha no ETL. Abortando...")
        return False
    
    # Fase 2: EDA
    print_header("FASE 2: EDA - Análise Exploratória de Dados")
    if not run_script('./03_Analise_Exploratoria/eda_pnad_covid.py', 'EDA (Gráficos exploratórios)'):
        print_error("Falha no EDA. Abortando...")
        return False
    
    # Fase 3: SEIR
    print_header("FASE 3: SEIR - Modelo Epidemiológico")
    if not run_script('./04_Modelo_SEIR/modelo_seir.py', 'SEIR (Simulação de cenários)'):
        print_error("Falha no SEIR. Abortando...")
        return False
    
    # Cópiar arquivos SEIR para pasta central
    ...
    try:
        ...
        # Copiar métricas
        seir_metrics = Path('04_Modelo_SEIR/relatorios/metricas_seir_cenarios.csv')
    ...
    print(f"   2. Ler documentação:")
    print(f"      - README.md (visão geral)")
    print(f"      - 05_Relatorios/RELATORIO_EXECUTIVO.md")
    print(f"      - 05_Relatorios/RECOMENDACOES_HOSPITAL.md")
